...
public enum ConnectionProtocol implements BundlerInfo.Provider {
   ...
   public int getPacketId(PacketFlow p_265671_, Packet<?> p_265442_) {
      return this.flows.get(p_265671_).getId(p_265442_.getClass());
   }

   public BundlerInfo getBundlerInfo(PacketFlow p_265790_) {
      return this.flows.get(p_265790_).bundlerInfo();
   }
   ...
   static {
      for(ConnectionProtocol connectionprotocol : values()) {
         ...
         connectionprotocol.flows.forEach((p_264719_, p_264720_) -> {
            p_264720_.listAllPackets((p_195615_) -> {
   ...
   static class PacketSet<T extends PacketListener> {
      ...
      public void listAllPackets(Consumer<Class<? extends Packet<?>>> p_265213_) {
         this.classToId.keySet().stream().filter((p_264721_) -> {
            return p_264721_ != (Object)BundleDelimiterPacket.class;
         }).forEach(p_265213_);
         this.extraClasses.forEach(p_265213_);
      }
