...
public class CamelAi {
   ...
   private static void initCoreActivity(Brain<Camel> p_249998_) {
      p_249998_.addActivity(Activity.CORE, 0, ImmutableList.of(new Swim(0.8F), new CamelAi.CamelPanic(4.0F), new LookAtTargetSink(45, 90), new MoveToTargetSink(), new CountDownCooldownTicks(MemoryModuleType.TEMPTATION_COOLDOWN_TICKS), new CountDownCooldownTicks(MemoryModuleType.GAZE_COOLDOWN_TICKS)));
   }

   private static void initIdleActivity(Brain<Camel> p_252342_) {
      p_252342_.addActivity(Activity.IDLE, ImmutableList.of(Pair.of(0, SetEntityLookTargetSometimes.create(EntityType.PLAYER, 6.0F, UniformInt.of(30, 60))), Pair.of(1, new AnimalMakeLove(EntityType.CAMEL, 1.0F)), Pair.of(2, new FollowTemptation((p_250812_) -> {
         return 2.5F;
      })), Pair.of(3, BehaviorBuilder.triggerIf(Predicate.not(Camel::refuseToMove), BabyFollowAdult.create(ADULT_FOLLOW_RANGE, 2.5F))), Pair.of(4, new RandomLookAround(UniformInt.of(150, 250), 30.0F, 0.0F, 0.0F)), Pair.of(5, new RunOne<>(ImmutableMap.of(MemoryModuleType.WALK_TARGET, MemoryStatus.VALUE_ABSENT), ImmutableList.of(Pair.of(BehaviorBuilder.triggerIf(Predicate.not(Camel::refuseToMove), RandomStroll.stroll(2.0F)), 1), Pair.of(BehaviorBuilder.triggerIf(Predicate.not(Camel::refuseToMove), SetWalkTargetFromLookTarget.create(2.0F, 3)), 1), Pair.of(new CamelAi.RandomSitting(20), 1), Pair.of(new DoNothing(30, 60), 1))))));
   }
   ...
   public static class CamelPanic extends AnimalPanic {
      ...
      protected void start(ServerLevel p_249530_, PathfinderMob p_248643_, long p_251124_) {
         if (p_248643_ instanceof Camel camel) {
   ...
   public static class RandomSitting extends Behavior<Camel> {
      ...
      protected boolean checkExtraStartConditions(ServerLevel p_249520_, Camel p_250322_) {
         return !p_250322_.isInWater() && p_250322_.getPoseTime() >= (long)this.minimalPoseTicks && !p_250322_.isLeashed() && p_250322_.onGround() && !p_250322_.hasControllingPassenger();
      }
