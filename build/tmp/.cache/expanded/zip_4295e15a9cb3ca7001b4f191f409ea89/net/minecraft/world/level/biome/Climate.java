...
public class Climate {
   ...
   public static record Parameter(long min, long max) {
      public static final Codec<Climate.Parameter> CODEC = ExtraCodecs.intervalCodec(Codec.floatRange(-2.0F, 2.0F), "min", "max", (p_275164_, p_275165_) -> {
   ...
   public static class ParameterList<T> {
      ...
      public static <T> Codec<Climate.ParameterList<T>> codec(MapCodec<T> p_275523_) {
         return ExtraCodecs.nonEmptyList(RecordCodecBuilder.<Pair<Climate.ParameterPoint, T>>create((p_275233_) -> {
            return p_275233_.group(Climate.ParameterPoint.CODEC.fieldOf("parameters").forGetter(Pair::getFirst), p_275523_.forGetter(Pair::getSecond)).apply(p_275233_, Pair::of);
   ...
   protected static final class RTree<T> {
      ...
      private static <T> List<Climate.RTree.SubTree<T>> bucketize(List<? extends Climate.RTree.Node<T>> p_186945_) {
         ...
         int i = (int)Math.pow(6.0D, Math.floor(Math.log((double)p_186945_.size() - 0.01D) / Math.log(6.0D)));
