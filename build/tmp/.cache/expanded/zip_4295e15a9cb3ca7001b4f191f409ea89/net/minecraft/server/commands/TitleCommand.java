...
public class TitleCommand {
   public static void register(CommandDispatcher<CommandSourceStack> p_139103_) {
      ...
      }))).then(Commands.literal("times").then(Commands.argument("fadeIn", TimeArgument.time()).then(Commands.argument("stay", TimeArgument.time()).then(Commands.argument("fadeOut", TimeArgument.time()).executes((p_139105_) -> {
   ...
   private static int clearTitle(CommandSourceStack p_139109_, Collection<ServerPlayer> p_139110_) {
      ...
      if (p_139110_.size() == 1) {
         p_139109_.sendSuccess(() -> {
            return Component.translatable("commands.title.cleared.single", p_139110_.iterator().next().getDisplayName());
         }, true);
      } else {
         p_139109_.sendSuccess(() -> {
            return Component.translatable("commands.title.cleared.multiple", p_139110_.size());
         }, true);
      }
   ...
   private static int resetTitle(CommandSourceStack p_139125_, Collection<ServerPlayer> p_139126_) {
      ...
      if (p_139126_.size() == 1) {
         p_139125_.sendSuccess(() -> {
            return Component.translatable("commands.title.reset.single", p_139126_.iterator().next().getDisplayName());
         }, true);
      } else {
         p_139125_.sendSuccess(() -> {
            return Component.translatable("commands.title.reset.multiple", p_139126_.size());
         }, true);
      }
   ...
   private static int showTitle(CommandSourceStack p_142781_, Collection<ServerPlayer> p_142782_, Component p_142783_, String p_142784_, Function<Component, Packet<?>> p_142785_) throws CommandSyntaxException {
      ...
      if (p_142782_.size() == 1) {
         p_142781_.sendSuccess(() -> {
            return Component.translatable("commands.title.show." + p_142784_ + ".single", p_142782_.iterator().next().getDisplayName());
         }, true);
      } else {
         p_142781_.sendSuccess(() -> {
            return Component.translatable("commands.title.show." + p_142784_ + ".multiple", p_142782_.size());
         }, true);
      }
   ...
   private static int setTimes(CommandSourceStack p_139112_, Collection<ServerPlayer> p_139113_, int p_139114_, int p_139115_, int p_139116_) {
      ...
      if (p_139113_.size() == 1) {
         p_139112_.sendSuccess(() -> {
            return Component.translatable("commands.title.times.single", p_139113_.iterator().next().getDisplayName());
         }, true);
      } else {
         p_139112_.sendSuccess(() -> {
            return Component.translatable("commands.title.times.multiple", p_139113_.size());
         }, true);
      }
