...
public class CommandBlockEditScreen extends AbstractCommandBlockEditScreen {
   ...
   protected void init() {
      super.init();
      this.modeButton = this.addRenderableWidget(CycleButton.<CommandBlockEntity.Mode>builder((p_287312_) -> {
         MutableComponent mutablecomponent;
         switch (p_287312_) {
            case SEQUENCE:
               mutablecomponent = Component.translatable("advMode.mode.sequence");
               break;
            case AUTO:
               mutablecomponent = Component.translatable("advMode.mode.auto");
               break;
            case REDSTONE:
               mutablecomponent = Component.translatable("advMode.mode.redstone");
               break;
            default:
   ...
   protected void populateAndSendPacket(BaseCommandBlock p_98384_) {
      this.minecraft.getConnection().send(new ServerboundSetCommandBlockPacket(BlockPos.containing(p_98384_.getPosition()), this.commandEdit.getValue(), this.mode, p_98384_.isTrackOutput(), this.conditional, this.autoexec));
   }
}