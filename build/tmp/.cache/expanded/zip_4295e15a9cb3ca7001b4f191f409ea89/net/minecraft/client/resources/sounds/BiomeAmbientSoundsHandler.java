...
public class BiomeAmbientSoundsHandler implements AmbientSoundHandler {
   ...
   public void tick() {
      ...
      if (biome != this.previousBiome) {
         ...
         biome.getAmbientLoop().ifPresent((p_263342_) -> {
      ...
      this.moodSettings.ifPresent((p_274718_) -> {
         ...
         BlockPos blockpos = BlockPos.containing(this.player.getX() + (double)this.random.nextInt(i) - (double)p_274718_.getBlockSearchExtent(), this.player.getEyeY() + (double)this.random.nextInt(i) - (double)p_274718_.getBlockSearchExtent(), this.player.getZ() + (double)this.random.nextInt(i) - (double)p_274718_.getBlockSearchExtent());
         int j = level.getBrightness(LightLayer.SKY, blockpos);
         if (j > 0) {
            this.moodiness -= (float)j / (float)level.getMaxLightLevel() * 0.001F;
         } else {
            this.moodiness -= (float)(level.getBrightness(LightLayer.BLOCK, blockpos) - 1) / (float)p_274718_.getTickDelay();
         }
         ...
         if (this.moodiness >= 1.0F) {
            ...
            SimpleSoundInstance simplesoundinstance = SimpleSoundInstance.forAmbientMood(p_274718_.getSoundEvent().value(), this.random, this.player.getX() + d3 / d6 * d7, this.player.getEyeY() + d4 / d6 * d7, this.player.getZ() + d5 / d6 * d7);
